...
def run_attention_generation_pipeline(model_id="Qwen/Qwen3.5-35B-A3B", base_output_workspace="coreai_workspace"):
    ...
    # Extract nested text config dimensions safely to dodge top-level attribute errors
    ...
    if os.path.exists(index_json_path):
        ...
        for layer_idx in range(num_layers):
            ...
            print(f"[Pipeline Hub] Triggering Layer {layer_idx} Attention Serialization Factory.")
            print(f"======================================================================")
            
           
            target_prefix_linear = f"model.language_model.layers.{layer_idx}.linear_attn."
            target_prefix_full = f"model.language_model.layers.{layer_idx}.self_attn."
            
            needed_files = set()
            for key, filename in weight_map.items():
                if key.startswith(target_prefix_linear) or key.startswith(target_prefix_full):
            ...
            if needed_files:
                ...
                for filename in list(needed_files):
                    ...
                    for k, v in partial_dict.items():
                        if k.startswith(target_prefix_linear) or k.startswith(target_prefix_full):
                            layer_state_dict[k] = v
                    del partial_dict
                
              
                convert_all_attentions_to_coreml_fp32(
                ...
                print(f"  [Skip Layer {layer_idx}] Weight nodes missing inside map indices schema arrays.")
